...
const EditNewsModal: React.FC<EditNewsModalProps> = ({ post, onClose }) => {
  const { updateNewsPost } = useAppContext();
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg shadow-xl p-8 w-full max-w-lg">
        <h2 className="text-2xl font-bold text-white mb-6">{t('admin.editNews.title')}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-300">{t('admin.editNews.formTitle')}</label>
            <input
              type="text"
              name="title"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300">{t('admin.editNews.formContent')}</label>
            <textarea
              name="content"
